---
title: "Recover position elements in ggrepel"
date: "`{r} Sys.Date()`"
knit: litedown:::knit
---
...
Also not available at `ggplot_gtable()`, where no labels have yet been materialized within the layer:
...
In the context of ggrepel, the positioning work is done inside `ggrepel:::makeContent.textrepeltree()` which also produces the actual layer grob, of class `textrepeltree`
...
::: {.side .side-right}
This constraint is actually not entirely accurate - you could technically convert npc coordinates back into data space by back-transforming with plot scales in `p$layout$panel_scales_(x|y)`. But that's a more brittle and involved process.
:::
