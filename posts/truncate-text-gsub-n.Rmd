...
If you wanted to add a trailing ellipse, you'd use `paste()` on that output:
...
::: {.side .side-right}
Oddly enough, the assignment variant `` `substr<-`(ipsum, 31, nchar(ipsum), "...") `` fails to achieve this effect. What you get instead is "..." masking a span of same length, ranging index 31-to-33 of the string.
:::

But if you use `gsub()` instead, you can express that in a single line:
